...
While you can always download project files manually as ZIP files from the course assignments page, and manually extract and place them in your `cs220` folder, you can also use Pixi to download the CS220 project files to automate this process.
...
```bash
Your project is placed at : /Users/<username>/Desktop/cs220/p1
```

This means that the project files have been downloaded and extracted into a new folder named `p1` inside your `cs220` folder.
...
For future projects, you can use similar commands (for example, `pixi run get_project p2`). Each project will be downloaded into its own separate folder inside `cs220`.
